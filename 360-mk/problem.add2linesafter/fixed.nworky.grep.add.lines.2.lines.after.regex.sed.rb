...
# https://stackoverflow.com/questions/28119359/how-do-i-add-a-line-after-another-line-in-a-file-in-ruby#


# ref: C:\n\Dropbox\csd\serve\rmksh\360-mk\r360f-mk-activestorage.sh
...
<p>
  ...
  ARGF.each_with_index do |line, ndx| 
    puts line
    # find line number of line matching.
    if line =~ /#{Regexp.escape(patrn)}/
      i = ndx + 1  # calculate offset where to insert new lines
       # puts ndx
       # puts i
    end
    # puts i
    # puts ndx
    # Insert the new lines at the calculated line number.
    if ndx == i
        # puts i
        # puts ndx
        puts repl2 
...
HEREDOC
ruby $r1tmp $filetarg > $filetarg.tmp ; cat $filetarg.tmp ; 

cp $filetarg.tmp $filetarg.2; 
...
# usage:
#
#   appn='rac360f3'; chmod +x  ./r360f-mk-activestorage.sh ; ./r360f-mk-activestorage.sh 2>&1 | tee -a $appn.sh_log$(date +"__%Y-%m-%d_%H.%M.%S").log



# see usages, not here.. appn='rac360f3'
# set appn above.
